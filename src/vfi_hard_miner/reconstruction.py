"""Deterministic CPU reconstruction for midpoint frame interpolation.

The production model returns backward flows and two already-sigmoided masks.
This module deliberately performs the reconstruction on CPU in float32.  That
keeps the first implementation independent from NPU ``grid_sample`` support
and gives CUDA/NPU runs one common numerical reference.
"""
...
class ReconstructionResult:
    """Original-resolution CPU float32 tensors used for diagnosis."""
...
def resize_backward_flow(
    ...
    *,
    ...
    flow_cpu = _as_cpu_float32(flow, "flow")
    _validate_nchw(flow_cpu, "flow", channels=2)
...
def resize_mask(
    ...
    *,
    ...
    mask_cpu = _as_cpu_float32(mask, "mask")
    _validate_nchw(mask_cpu, "mask", channels=1)
    _validate_unit_interval(mask_cpu, "mask")
    out_h, out_w = _validate_hw(output_size, "output_size")
    resized = F.interpolate(
        mask_cpu,
        size=(out_h, out_w),
...
def backward_warp(
    ...
    *,
    ...
    image_cpu = _as_cpu_float32(image, "image")
    flow_cpu = _as_cpu_float32(backward_flow, "backward_flow")
    _validate_nchw(image_cpu, "image", channels=3)
    _validate_nchw(
        flow_cpu,
        "backward_flow",
        channels=2,
        batch=image_cpu.shape[0],
        spatial=tuple(image_cpu.shape[-2:]),
    )

    batch, _, height, width = image_cpu.shape
    y_base, x_base = torch.meshgrid(
        torch.arange(height, dtype=torch.float32),
        torch.arange(width, dtype=torch.float32),
        indexing="ij",
    )
    x = x_base.unsqueeze(0).expand(batch, -1, -1) + flow_cpu[:, 0]
    y = y_base.unsqueeze(0).expand(batch, -1, -1) + flow_cpu[:, 1]
...
def reconstruct_midpoint(
    ...
    *,
    ...
    image0 = _as_cpu_float32(img0, "img0")
    image1 = _as_cpu_float32(img1, "img1")
    ...
    raw_flow0 = _as_cpu_float32(flow_t0, "flow_t0")
    raw_flow1 = _as_cpu_float32(flow_t1, "flow_t1")
    raw_mask0 = _as_cpu_float32(mask0, "mask0")
    raw_mask1 = _as_cpu_float32(mask1, "mask1")
    ...
    resized_mask0 = resize_mask(raw_mask0, original_size, align_corners=align_corners)
    resized_mask1 = resize_mask(raw_mask1, original_size, align_corners=align_corners)
